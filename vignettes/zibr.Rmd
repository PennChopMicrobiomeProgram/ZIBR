...
```{r}
zibr.fit <- zibr(
  logistic.cov = sim$X, beta.cov = sim$X, Y = sim$Y,
  subject.ind = sim$subject.ind, time.ind = sim$time.ind
)
zibr.fit
```
...
## Real Data (from [paper](http://bioinformatics.oxfordjournals.org/content/early/2016/05/14/bioinformatics.btw308.short?rss=1))
...
for (spe in spe.all) {
  ...
  lme.fit <- lme4::lme(Y.tran ~ Baseline + Time + Treat, random = ~ 1 | SID, data = tdata)
...
setdiff(
  p.species.lme.adj[p.species.zibr.adj$Time < 0.05, "Species"],
